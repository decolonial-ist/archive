# APT28 Nearest Neighbor Campaign

**Автор:** MITRE ATT&CK®
**Год публикации:** 2025
**Описывает российскую колонизацию народов:** Украинцы
**Права на публикацию:** свободно (MITRE ATT&CK Terms of Use: перепубликация разрешена при сохранении копирайта MITRE)

Страница витрины: https://archive.decolonial.ist/36

## Что здесь лежит

| файл | что это | объём | где |
|---|---|---|---|
| [apt28_nearest_neighbor_campaign_2025.pdf](https://github.com/decolonial-ist/archive/blob/main/36/apt28_nearest_neighbor_campaign_2025.pdf) | PDF | 42 КБ | в этой папке |
| [apt28_nearest_neighbor_campaign_2025.txt](https://github.com/decolonial-ist/archive/blob/main/36/apt28_nearest_neighbor_campaign_2025.txt) | текст | 5 КБ | в этой папке |

## Откуда это у нас

Первоисточник: [https://attack.mitre.org/campaigns/C0051/](https://attack.mitre.org/campaigns/C0051/)

Текст расшифрован нами со скана либо сохранён со страницы первоисточника; исходный файл лежит рядом, чтобы расшифровку можно было сверить.

## Где упоминается в базе знаний

Сноски на этот материал стоят в 1 месте.

### Процедуры

- **[Полномасштабное вторжение (с 24 февраля 2022)](https://decolonial.ist/campaigns/C0103/#procedures) — техника [Кибероперации](https://decolonial.ist/techniques/T0136/)**
  > Масштабные операции спецслужб РФ по кибершпионажу, перехвату данных (в том числе через взлом публичных сетей Wi-Fi) и скрытому развертыванию деструктивного ПО против украинских организаций и граждан


---

без куки и трекинга, статистика — <a href="https://www.goatcounter.com" rel="noopener">GoatCounter</a> · контент доступен по лицензии <a href="https://creativecommons.org/licenses/by/4.0/deed.ru" rel="noopener">CC BY 4.0</a> · страницы собраны автоматически конвейером архива [decolonial.ist](https://decolonial.ist). Картотека — в [корне репозитория](../README.md).
