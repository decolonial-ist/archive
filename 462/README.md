# Полное собрание законов Российской империи. Собрание третье. Том XI (1891)

**Автор:** Александр III
**Год публикации:** 1894
**Где напечатано:** указ № 7865 от 15 июня 1891 «Об отчуждении земель, потребных для сооружения Беслан-Петровской железной дороги» и мнение Госсовета № 7867 «О дополнении к Уставу Общества Владикавказской железной дороги»; ПСЗРИ
**Описывает российскую колонизацию народов:** Нохчи (Чеченцы)
**Права на публикацию:** свободно (общественное достояние: издано в 1894 году)

Страница витрины: https://archive.decolonial.ist/462

## Что здесь лежит

| файл | что это | объём | где |
|---|---|---|---|
| [polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894.txt](https://github.com/decolonial-ist/archive/blob/main/462/polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894.txt) | текст | 7.2 МБ | в этой папке |
| [polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894_layer.txt](https://github.com/decolonial-ist/archive/blob/main/462/polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894_layer.txt) | текст | 7.3 МБ | в этой папке |
| [polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894.pdf](https://github.com/decolonial-ist/archive/releases/download/originals-v2/462__polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894.pdf) | PDF | 138.1 МБ | актив релиза (файл тяжелее 10 МБ) |

## Откуда это у нас

Первоисточник: [https://archive.org/details/polnoesobranieza1118russ](https://archive.org/details/polnoesobranieza1118russ)

Текст расшифрован нами со скана либо сохранён со страницы первоисточника; исходный файл лежит рядом, чтобы расшифровку можно было сверить.

## Цифровые отпечатки файлов

| файл | sha256 |
|---|---|
| polnoe_sobranie_zakonov_rossiyskoy_imperii_sobranie_trete_tom_xi_1891_1894.pdf | `af63f5f08090ca84c8358fbf87d52916655a5998c417649ea1e6a022e5fb8f58` |

## Где упоминается в базе знаний

Сноски на этот материал стоят в 2 местах.

### Кампании

- **[Земельное ограбление: колониальный передел земли, малоземелье и казачья колонизация Терской области (1860-1917)](https://decolonial.ist/campaigns/C1171/)** (на материале стоит связь с другой записью базы)

### Процедуры

- **[Земельное ограбление: колониальный передел земли, малоземелье и казачья колонизация Терской области (1860-1917)](https://decolonial.ist/campaigns/C1171/#procedures) — техника [Изъятие ресурсов](https://decolonial.ist/techniques/T0052/)**
  > 15 июня 1891 года Александр III именным указом министру путей сообщения предписал изъятие земель под трассу: «Разрѣшивъ сооруженіе Петровской желѣзной дороги, взамѣнъ ранѣе указаннаго направленія ея отъ Владикавказа до Петровска, по направленію отъ станціи Бесланъ... до Петровска, повелѣваемъ: 1) сдѣлать надлежащія распоряженія къ отчужденію... необходимыхъ для сей цѣли земель»


---

без куки и трекинга, статистика — <a href="https://www.goatcounter.com" rel="noopener">GoatCounter</a> · контент доступен по лицензии <a href="https://creativecommons.org/licenses/by/4.0/deed.ru" rel="noopener">CC BY 4.0</a> · страницы собраны автоматически конвейером архива [decolonial.ist](https://decolonial.ist). Картотека — в [корне репозитория](../README.md).
